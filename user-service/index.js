...
app.post('/users', (req, res) => {
  ...
  try {
    db.prepare('INSERT INTO users (id,full_name,email,api_key,created_at) VALUES (?,?,?,?,?)')
      .run(id, fullName.trim(), email?.trim() || null, key, now);
  } catch(e) {
...
// GET /internal/users/by-api-key/:key  (gateway auth)
app.get('/internal/users/by-api-key/:key', (req, res) => {
  const u = db.prepare('SELECT * FROM users WHERE api_key=?').get(req.params.key);
